...
Reproduction: `yarn test:kickback --grep 'one real promoter'`. The assertions for checkout, attribution, ledger and wallet display passed; financial teardown failed. A subsequent read confirmed the discrepancy, and the three test events were hidden while retaining their orders and ledger. [Sanitized event evidence](kickback-refund-evidence.json) includes the IDs for investigation. No withdrawal of those test earnings was requested. A subsequent focused US-account settlement run completed automatic transfers and old-wallet withdrawals in USD, CAD and EUR, then reproduced missing commission reversals after successful ticket refunds. The earlier evidence describes unwithdrawn wallets; the later transfer evidence is in [current results](kickback-results.json).
...
**Local fix:** `TransactionsService.CompleteTransactionAsync` now uses the transaction's saved checkout ticket totals when present, preserving the commission allocations, prices and holder association instead of accepting a callback's recalculated totals. Legacy transactions without saved totals retain the existing fallback. Six regression cases exercise real pricing recalculation, settlement allocation, completion and ticket generation for USD/CAD/EUR with both Kickback and organizer-assigned attribution; all six reproduced zero allocations before the fix and pass afterward. The refund matrix covers 18 full/partial refund combinations across those currencies and wallet/automatic/withdrawn earnings. All 474 selected backend tests pass, including adjacent checkout flows. Repositories and provider calls are faked in these service tests; testlab Stripe verification still requires deployment. Previously issued tickets and refunds are not repaired automatically, and the separate refund-before-ledger defect in finding 2 remains outside this fix. Reports: `/private/tmp/kickback-refund-fix/before.trx` and `/private/tmp/kickback-refund-fix/verified.trx`.
...
Current reproduction: `yarn test:kickback --grep "another authenticated user receives forbidden"` in `testlab_api.spec.ts`. Historical reproduction: `ReportingTests.Foreign_promoter_requests_are_denied_by_the_real_HTTP_pipeline(resource: "payments")`. [The controller](/Users/christianjaimes/dev/jass/e2e/jasstickets-api/JassTicketsApi/Controllers/PrivateEventPromotersController.cs:394) previously passed the explanatory message to `Forbid(string)`, where ASP.NET interprets it as an authentication-scheme name.

**Local fix:** return an explicit HTTP 403 with the message. The strengthened controller regression executes the MVC result using the real JWT authentication service: it reproduced the original scheme exception before the fix and returns 403 afterward, without querying payment history. A separate owner case preserves pagination/search/currency forwarding. All 35 `PrivateEventPromotersControllerTests` pass. The existing testlab Playwright regression remains enabled; rerun it after deploying the backend fix. Reports: `/private/tmp/kickback-history-403/before.trx` and `/private/tmp/kickback-history-403/after.trx`.
...
The latest testlab run also corrected three test assumptions: the organizer discount-code list excludes buyer codes by design; hiding an already-hidden event returns 304; and requesting a valid currency before a promoter profile exists returns 404. All three corrected cases passed their focused rerun. That earlier run stopped settlement tests at a missing-deployment preflight (404). Both routes are now deployed. In CI attempt 2, all three settlement cases instead hit the optional date-of-birth modal, and two API CI cases started their account timeout too early. These test defects are corrected locally; they are separate from the refund and foreign-history product bugs reproduced in both CI runs. See [current execution results](kickback-results.json).
