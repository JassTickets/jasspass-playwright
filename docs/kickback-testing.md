...
Updated workflows are in the Playwright repository (`.github/workflows/playwright.yml`), UI repository (`.github/workflows/vercel-test-cicd.yml`) and API repository (`.github/workflows/deploy-test.yml`). Discovery currently includes all 54 spec files (456 project-test instances), including 73 Kickback cases. Each job explicitly sets `JASS_TEST_URL` to testlab, uses the lockfile, and retains HTML reports plus traces/screenshots on failure. Matrix jobs use `fail-fast: false` so a failing suite does not cancel the others. The API's Playwright jobs wait for both the server and edge-worker deployments. Existing application unit-test jobs remain separate.

These are working-tree changes; they take effect on GitHub after the respective repository changes are merged/pushed. Deployment workflows check out the Playwright repository's default branch, so the test/configuration changes must reach that branch before workflows select the new `kickback` project. No GitHub run or application deployment was dispatched while implementing this correction.
...
**Both application changes must be deployed to testlab before the three settlement cases can run.** Current testlab returns 404 at their deployment check; all three fail visibly before creating paid fixtures. Once the Playwright changes reach its default branch and the API/UI changes are deployed, all three existing CI workflows select these tests automatically. No push, merge or deployment has been dispatched.
